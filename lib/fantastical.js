(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["fantastical"] = factory();
	else
		root["fantastical"] = factory();
...
const capitalize = __webpack_require__(0);
const sample = __webpack_require__(1);
const endsWith = __webpack_require__(6);
...
const _names = {
    ...
    doughty: {
        syllables: ['bal', 'durn', 'na', 'bord', 'from', 'nor', 'born', 'fror', 'nord', 'brim', 'fuld', 'orm', 'brod', 'fund', 'skand', 'brokk', 'gim', 'skond', 'brom', 'glo', 'storn', 'bru', 'gond', 'strom', 'bur', 'gord', 'stur', 'burl', 'gorm', 'sturl', 'da', 'grad', 'sund', 'dal', 'grim', 'thor', 'dolg', 'grod', 'thorn', 'dor', 'grom', 'thra', 'dorm', 'guld', 'thro', 'dral', 'gund', 'throl', 'drim', 'gur', 'thror', 'drom', 'hord', 'thru', 'dur', 'horn', 'thrur', 'durm', 'hra', 'thund'],
    ...
    homely: {
        syllables: ['ad', 'blanc', 'falc', 'mil', 'adel', 'boff', 'ferd', 'mung', 'adr', 'bomb', 'frob', 'od', 'ail', 'bram', 'fulb', 'oth', 'alb', 'bung', 'gam', 'sab', 'alm', 'droc', 'hald', 'sam', 'amb', 'drog', 'ham', 'seg', 'band', 'durl', 'hasc', 'serl', 'bard', 'emm', 'hod', 'tob', 'ben', 'erd', 'hug', 'wan', 'biff', 'ern', 'iv', 'wig', 'bild', 'ever', 'mark', 'wyd'],
...
exports.dwarf = (gender) => {
    const nameArr = _names.doughty;
    let name = capitalize(sample(nameArr.syllables));
    const roll = helpers_1.dieRoll(10);
    if (gender === 'male') {
        if (roll > 7)
            name = `${name}${helpers_1.isVowel(name.slice(-1)) ? 'r' : 'i'}`;
        else
            name = `${name}${sample(nameArr.maleSuffixes)}`;
    }
    else if (gender === 'female') {
        if (roll > 7)
            name = `${name}${helpers_1.isVowel(name.slice(-1)) ? 'ra' : 'a'}`;
        else
            name = `${name}${sample(nameArr.femaleSuffixes)}`;
    }
    return name;
};
exports.halfling = (gender) => {
    const nameArr = _names.homely;
    let name = capitalize(sample(nameArr.syllables));
    const roll = helpers_1.dieRoll(10);
    if (nameArr[`${gender}Suffixes`]) {
        name = `${name}${sample(nameArr[`${gender}Suffixes`])}`;
    }
    if (roll > 6)
        name = `${name} ${sample(_names.familyName.english)}`;
    return name;
};
exports.gnome = (gender) => {
    let name = capitalize(sample(_names.doughty.syllables));
    const roll = helpers_1.dieRoll(10);
    if (_names.homely[`${gender}Suffixes`]) {
        name = `${name}${sample(_names.homely[`${gender}Suffixes`])}`;
    }
    if (roll > 6)
        name = `${name} ${sample(_names.familyName.scottish)}`;
    return name;
};
exports.elf = (gender) => {
    const nameArr = _names.fairAndNoble;
...
exports.fairy = (gender) => {
    const nameArr = _names.fairy;
...
exports.highfairy = (gender) => {
    const nameArr = _names.alternateFairy;
    let name = capitalize(sample(nameArr.prefixes));
    if (nameArr[`${gender}Suffixes`]) {
        name = `${name}${sample(nameArr[`${gender}Suffixes`])}`;
    }
...
exports.darkelf = (gender) => {
    const nameArr = _names.elegantEvil;
...
exports.halfdemon = (gender) => {
    const nameArr = _names.malevolent;
    let name = capitalize(sample(nameArr.prefixes));
    if (nameArr[`${gender}Suffixes`]) {
        name = `${name}${sample(nameArr[`${gender}Suffixes`])}`;
    }
...
exports.demon = () => {
    const keys = Object.keys(_names.infernal);
    const roll = helpers_1.dieRoll(3) - 1;
    const roll2 = helpers_1.dieRoll(2) - 1;
    const chosen = keys[roll];
    const chosen2 = keys.filter(x => x !== chosen)[roll2];
    return `${capitalize(sample(_names.infernal[chosen]))}${sample(_names.infernal[chosen2])}`;
};
exports.dragon = (gender) => {
    const nameArr = _names.draconic;
    let name = capitalize(sample(nameArr.prefixes));
...
exports.angel = (gender) => {
    const nameArr = _names.empyreal;
    const roll = helpers_1.dieRoll(12);
    let name = capitalize(sample(nameArr.prefixes));
    if (roll === 1) {
        if (gender === 'female') {
...
exports.human = ({ allowMultipleNames } = { allowMultipleNames: false }) => {
    const roll = helpers_1.dieRoll(20);
    if (roll < 3)
        return sample(_names.human.one);
    if (roll < 12)
        return sample(_names.human.two);
    if (roll < 17)
        return sample(_names.human.three);
    if (roll === 17)
        return sample(_names.human.more);
    if (roll === 18 && allowMultipleNames)
        return `${sample(_names.human.one)} ${sample(_names.human.two)}`;
    if (roll === 19 && allowMultipleNames)
        return `${sample(_names.human.two)} ${sample(_names.human.one)}`;
    if (roll === 20 && allowMultipleNames) {
        const keys = Object.keys(_names.human);
        return `${sample(_names.human[sample(keys)])} ${sample(_names.human[sample(keys)])}`;
    }
    return sample(_names.human[sample(Object.keys(_names.human))]);
};
exports.species = { goblin: exports.goblin, orc: exports.orc, ogre: exports.ogre, cavePerson: exports.cavePerson, dwarf: exports.dwarf, halfling: exports.halfling, gnome: exports.gnome, elf: exports.elf, highelf: exports.highelf, fairy: exports.fairy, highfairy: exports.highfairy, darkelf: exports.darkelf, drow: exports.drow, halfdemon: exports.halfdemon, demon: exports.demon, dragon: exports.dragon, angel: exports.angel, human: exports.human };
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.dieRoll = (dieSize = 10) => Math.floor(Math.random() * dieSize) + 1;
...
Object.defineProperty(exports, "__esModule", { value: true });
const includes = __webpack_require__(9);
exports.isVowel = (letter) => includes(['a', 'e', 'i', 'o', 'u'], letter);
...
const sample = __webpack_require__(1);
const helpers_1 = __webpack_require__(4);
const species_1 = __webpack_require__(2);
const _mysticOrder = {
    patterns: [
        '<group> of the <entity>',
...
exports.mysticOrder = () => {
    const config = {
        group: _mysticOrder.group[helpers_1.dieRoll(10) > 5 ? 'cliques' : 'people'],
...
exports.militaryUnit = () => {
    const config = {
        commander: [species_1.human()],
...
exports.guild = () => {
    const roll = helpers_1.dieRoll(10);
    if (roll < 3)
        return `${capitalize(sample(_guilds.roles))} of ${capitalize(sample(_guilds.goals))}`;
    if (roll < 6)
        return `${capitalize(sample(_guilds.adjectives))} ${capitalize(sample(_guilds.actions))} ${capitalize(sample(_guilds.titles))}`;
    return `${capitalize(sample(_guilds.descriptions))} ${capitalize(sample(sample(_guilds.groups)))}`;
};
exports.parties = { mysticOrder: exports.mysticOrder, militaryUnit: exports.militaryUnit, guild: exports.guild };
...
exports.places = { tavern: exports.tavern };
...
exports.adventures = { adventure: exports.adventure };
